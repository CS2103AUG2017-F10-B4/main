...
* if search by `phone number` then `PhoneContainsSpecifiedKeywordsPredicate` is used
* if search by `email` then `EmailContainsSpecifiedKeywordsPredicate` is used
...
image::AddSortSequenceDiagram.png[width="800"]
...
image::imagebutton.png[width="800"]

The user can use of file 'png' or 'jpg'.
...
The `RemarkCommand` object sequence diagram can be seen below:

image::RemarkLogicSequenceDiagram.png[width="750"]
_Figure 3.14.1.1 : RemarkCommand parser sequence diagram_

As we can see, `RemarkCommand` object is created after input has been validated from the `AddressBookParser`.
