...
Alternative considered is to allow user input multiple details in each search. It may help to narrow down and shorten the list of contacts that will be returned, but it is not useful as user may only recall some details of a person only and thus not implemented . For example:
...
From the two scenarios, we know that unless the user knows the exact details (i.e. full name, exact phone number, exact email and etc) otherwise this `finds` command is practically not useful as no results will be returned.
...
|`* * *` |user |edit my addressBook |make changes to the addressBook

|`* * *` |user |view my contact list in alphabetical order |scroll through the list easily

|`* * *` |user |find persons by birthday month |find who is having birthdays

|`* * *` |user |add a color for person cards | give different colors to different person

|`* * *` |user |pin contacts at the top |find my favourite contacts more easily

|`* *` |user |have confirmation before deleting a person |ensure that I am deleting the correct person

| `* *` |user |delete persons that contain a certain tag |delete a group with convenience

|`*` |user |be able to add and view money lent to my friends |keep track of money lending

|`*` |user |open one help UI at any time |limit the number of windows opened

|`*` |user |see the time |keep track of the time

|`*` |user |apply different themes |customise it according to my preference
